...
def main():
    ...
    log = open(os.path.join(args.save_path,
                            'seed-{}-{}.log'.format(args.manualSeed, time_for_file())), 'w')
    ...
    train_transform += [transforms.TrainScale2WH(
        (args.crop_width, args.crop_height))]
    train_transform += [transforms.AugScale(
        args.scale_prob, args.scale_min, args.scale_max)]
    # if args.arg_flip:
    #  train_transform += [transforms.AugHorizontalFlip()]
    if args.rotate_max:
        train_transform += [transforms.AugRotate(args.rotate_max)]
    train_transform += [transforms.AugCrop(args.crop_width,
                                           args.crop_height, args.crop_perturb_max, mean_fill)]
    ...
    train_data = datasets.GeneralDataset(
        train_transform, args.sigma, args.downsample, args.heatmap_type, args.dataset_name)
    ...
    if args.eval_lists is not None:
        for eval_list in args.eval_lists:
            eval_data = datasets.GeneralDataset(
                eval_transform, args.sigma, args.downsample, args.heatmap_type, args.dataset_name)
            ...
            eval_loader = torch.utils.data.DataLoader(
                eval_data, batch_size=args.eval_batch, shuffle=False, num_workers=args.workers, pin_memory=True)
    ...
    args.modelconfig = models.ModelConfig(
        train_data.NUM_PTS+1, args.cpm_stage, args.pretrain, args.argmax_size)
    ...
    if args.cycle_model_path is None:
        ...
        print_log(
            'itn-parameters : {:} MB'.format(itnetwork.num_parameters()), log)
    ...
    assert osp.isdir(args.cycle_model_path), '{:} does not exist or is not dir.'.format(
        args.cycle_model_path)

    # start train itn-cpm model
    itn_cpm = models.__dict__[args.arch](
        args.modelconfig, args.cycle_model_path)
